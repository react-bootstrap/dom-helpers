 ...
 */
export default function removeClass(element: Element | SVGElement, className: string) {
  if (element.classList) {
    element.classList.remove(className);
  } else if (typeof element.className === 'string') {
    (element as Element).className = replaceClassName(element.className, className);
  } else {
